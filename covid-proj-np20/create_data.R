...
post_p20$min = NULL
post_p20$PovertyType = "Global P20"
post = rbind(post_ext, post_p20)

pre_global = subset(pre,Level=="Global" & PovertyType %in% c("Extreme poverty", "Global P20") & variable=="HeadCount")
post_global = subset(post,Level=="Global" & PovertyType %in% c("Extreme poverty", "Global P20") & variable=="HeadCount")
...
pre_national = subset(pre,Level %in% c("National", "Regional") & PovertyType %in% c("Extreme poverty", "Global P20") & variable=="HeadCount")
post_national = subset(post,Level %in% c("National", "Regional") & PovertyType %in% c("Extreme poverty", "Global P20") & variable=="HeadCount")
...
pre_p20_thresh$PovertyType = "Global P20"
...
post_p20_thresh$PovertyType = "Global P20"
...
all = subset(all,name %in% unique(all_2011$name))

all = rbind(all_2011,all_2017,all)
